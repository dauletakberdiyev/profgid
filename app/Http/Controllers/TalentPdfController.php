...
class TalentPdfController extends Controller
{
    public function download(Request $request)
    {
        ...
        foreach ($topTenTalents as $talent) {
            $talentAdvice[$talent['name']] = app('App\\Livewire\\Pages\\TalentTestResults')->getTalentAdvice($talent['name']);
        }
        $domainColors = [
            'executing' => '#702B7C',
            'relationship' => '#316EC6',
        ...
        $fileName = 'Отчет_' . $userName . '_' . now()->format('Y-m-d') . '.pdf';
        // Данные для разных вкладок
        if ($tab === 'talents') {
            // Все таланты, домены, топ-таланты, советы и т.д.
            $pdf = PDF::loadView('pdf.talent-full', [
            ...
            foreach ($allSpheres as $sphere) {
                ...
                if ($sphereTalents->count() > 0) {
                    ...
                    if ($totalWeight > 0) {
                        ...
                        $coverageBonus = ($matchingTalentsCount / $sphereTalents->count()) * 10;
                        $compatibilityPercentage = min($baseCompatibility + $coverageBonus, 100);
                    }
                }
                $spheresData->push([
                    'id' => $sphere->id,
            ...
            foreach ($allProfessions as $profession) {
                ...
                if ($profession->talents && $profession->talents->count() > 0) {
                    ...
                    if ($totalWeight > 0) {
                        $baseCompatibility = ($totalWeightedScore / $totalWeight) * 100;
                        $coverageBonus = ($matchingTalentsCount / $profession->talents->count()) * 10;
                        $compatibilityPercentage = min($baseCompatibility + $coverageBonus, 100);
                    }
                }
            ...
            $pdf = PDF::loadView('pdf.spheres-full', [
            ...
            $pdf = PDF::loadView('pdf.professions-full', [
                'topProfessions' => $topProfessions,
                'testDate' => $testDate,
                // ...
            ]);
